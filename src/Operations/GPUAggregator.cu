...
// #include <climits>
// #include <cfloat> // For DBL_MAX alternative
...
//     if (tid == 0) {
//         atomicAdd(result, shared_sum[0]); // Requires compute capability >= 6.0 for double
//     }
// }
...
//         if (is_min) {
//             if (shared_val[0] < current) *result = shared_val[0];
//         } else {
//             if (shared_val[0] > current) *result = shared_val[0];
//         }
//     }
